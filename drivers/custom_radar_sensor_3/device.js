...
const LOG_MARKER = '[CUSTOM_RADAR_SENSOR_3]';
...
class CustomRadarSensor3 extends TuyaSpecificClusterDevice {
  async onNodeInit({ zclNode }) {
    ...
    if (this.isFirstInit && this.isFirstInit()) {
    ...
    if (zclNode.endpoints[1].clusters[CLUSTER.ILLUMINANCE_MEASUREMENT.NAME]) {
  ...
  onIlluminanceMeasuredAttributeReport(measuredValue) {
    const luxValue = Math.round(measuredValue / 100);
  ...
  async _applyAllSettings() {
    ...
    const detectionDistanceCm = this._clampInt(settings.mw_radar_detection_distance, 0, 1000);
  ...
  async onSettings({ newSettings, changedKeys }) {
    try {
      ...
      if (changedKeys.includes('mw_radar_detection_distance')) {
        const value = this._clampInt(newSettings.mw_radar_detection_distance, 0, 1000);
  ...
  onDeleted() {
    this.log(LOG_MARKER, 'removed');
  }
}

module.exports = CustomRadarSensor3;
